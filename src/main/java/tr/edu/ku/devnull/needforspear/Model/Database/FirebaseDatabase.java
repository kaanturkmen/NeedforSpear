...
public class FirebaseDatabase implements GameDatabase {
    ...
    private void sendNewVerification(DataSnapshot dataSnapshot, String email) {
        ...
        for (DataSnapshot unit : dataSnapshot.getChildren()) {
            ...
            if (currentPlayer.getAccount().getEmail().equals(email)) {
                ...
                JOptionPane.showMessageDialog(NeedforSpearGame.getMainFrame(), Constants.UIConstants.NEW_VERIFICATION_TEXT, "Alert", JOptionPane.WARNING_MESSAGE);
    ...
    private Integer checkIfValidCredentials(Player p1, Player p2) {
        ...
        if (!p2.getAccount().isVerified()) {
            JOptionPane.showMessageDialog(NeedforSpearGame.getMainFrame(), Constants.UIConstants.VERIFY_BEFORE_LOGIN_TEXT, Constants.UIConstants.ALERT_TEXT, JOptionPane.WARNING_MESSAGE);
        ...
        if (p1.getAccount().getPassword().equals(p2.getAccount().getPassword())) {
            ...
            JOptionPane.showMessageDialog(NeedforSpearGame.getMainFrame(), Constants.UIConstants.INCORRECT_PASSWORD_TEXT, Constants.UIConstants.ALERT_TEXT, JOptionPane.WARNING_MESSAGE);
            return DatabaseCredentials.DATABASE_FAIL;
        }
    }
    ...
    private void checkIfUserExistsAndRegister(DataSnapshot dataSnapshot, Player player) {
        ...
        for (DataSnapshot unit : dataSnapshot.getChildren()) {
            currentPlayer = unit.getValue(Player.class);
            if (currentPlayer.getAccount().getEmail().equals(player.getAccount().getEmail()) || currentPlayer.getAccount().getUsername().equals(player.getAccount().getUsername())) {
                JOptionPane.showMessageDialog(NeedforSpearGame.getMainFrame(), Constants.UIConstants.USER_EXISTS_TEXT, Constants.UIConstants.ALERT_TEXT, JOptionPane.WARNING_MESSAGE);
