...
public class BuildModeHandler {
    ...
    public void prepGameMap() {
        NeedforSpearGame.getInstance().setGameMap(new GameMap(new Size(Constants.UIConstants.INITIAL_SCREEN_WIDTH, Constants.UIConstants.INITIAL_SCREEN_HEIGHT)));
        obstacleList = NeedforSpearGame.getInstance().getGameMap().getListofObstacles();
    }
    ...
    public void setObstacleLocation(int startLocationY, int mapWidth) {
        Obstacle firstObstacle = NeedforSpearGame.getInstance().getGameMap().getListofObstacles().get(0);
        ...
        int j = 0;
        int m = 1;
        for (int i = 1; i < NeedforSpearGame.getInstance().getGameMap().getListofObstacles().size(); i++) {
            k = (Constants.UIConstants.OBSTACLE_HGAP + firstObstacle.getSize().getWidth()) * m + firstObstacle.getLocation().getXCoordinates();
            Obstacle nextObstacle = NeedforSpearGame.getInstance().getGameMap().getListofObstacles().get(i);
    ...
    public void createObstacles(int simpNum, int firmNum, int expNum, int giftNum) {
        ...
        for (i = 0; i < simpNum; i++) {
            ...
            NeedforSpearGame.getInstance().getGameMap().getListofObstacles().add(obstacle);
        }
        for (i = 0; i < firmNum; i++) {
        ...
        Collections.shuffle(NeedforSpearGame.getInstance().getGameMap().getListofObstacles());
    }
    ...
    public void removeObstacle(int x, int y, Graphics graphics, Obstacle obstacle) {
        ...
        NeedforSpearGame.getInstance().getGameMap().getListofObstacles().remove(obstacle);
    }
